...
test_complete_M$Time <- test_complete_M$Time %>% str_extract(pattern = "^\\d{1}:\\d{2}") %>%  #\d match any char with cat. number decimal digit
  hm()
...
```{r}
#The top end of the data is already trimmed because we have a hard cap of 2H20M
#We need to trim the bottom (fastest) portion of the data because this contains a lot of
#errors, such as supposedly world-record breaking times. I will trim 10% off the bottom
Y <- Wdat %>% select(Time.y)
Y <- icdf(Y$Time.y,0.10)
Wdat <- Wdat[Wdat$Time.y > Y,]
Wdat
Y
```
...
#We need to trim the bottom (fastest) portion of the data because this contains a lot of
#errors, such as supposedly world-record breaking times. I will trim 10% off the bottom
Y <- Mdat %>% select(Time.y)
Y <- icdf(Y$Time.y,0.10)
Mdat <- Mdat[Mdat$Time.y > Y,]
```
...
all_data$Race <- as.factor(all_data$Race) #this is a factor not a number
all_data <- all_data[,-c(5,6,9)] #Remove unwanted columns
```
...
ggplot(by_year_sex, aes(x = DATE, y = mediantime, color = sex)) +
  ...
  ggtitle("Median race times over the years")
```


```{r}
sort(unique(data_w$PRCP))
```
...
```{r}
test_data_1 <- test_data %>% filter(Race != "1974" &Race != "1975")
```
...
```{r}
boxcoxResult = boxcox(Time.y ~ 0+Age+sex+Race, data =test_data_1, lambda = seq(-2,5,1.5))
lambda = boxcoxResult$x[which.max(boxcoxResult$y)]
test_data_1$yboxcox = (test_data_1$Time.y^lambda-1)/lambda
model.boxcox = lm(yboxcox ~ 0+Age+sex+Race,data = test_data_1)
```
