...
df_cluster_2 <- df_cleantown %>% select(Age, Time.y, Hometown)
#column_to_rownames(df_cluster)
#column_to_rownames(df_cluster, 'Hometown')


df_cluster_2$Age <- scale(df_cluster_2$Age)
df_cluster_2$Time.y <- scale(df_cluster_2$Time.y)
df_cluster_2
```
...
```{r}
cluster_sample <- df_cluster_2[sample(nrow(df_cluster_2),100),]
cluster_sample$Age <- scale(cluster_sample$Age)
cluster_sample$Time.y<- scale(cluster_sample$Time.y)

cluster_sample
#rownames(cluster_sample) = cluster_sample$Hometown
...
```{r}
Q <- fviz_cluster(list(data = cluster_sample[,1:2], cluster = sub_grp))
Q
```
...
```{r}
df_cleantown %>% group_by(Hometown) %>% select(Age, Hometown, Time.y)
```
