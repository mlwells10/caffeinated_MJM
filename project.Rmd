...
Wdat = Wdata %>% filter(Race != 2015)
Mdat = Mdata %>% filter(Race != 2015)
```



```{r}
Wdat = Wdat[(!is.na(Wdata$Age))&(!is.na(Wdata$Time)),] #removing NA in Age and Time observation
...
```{r}
Mdat = Mdat[(!is.na(Mdata$Age))&(!is.na(Mdata$Time)),] #removing NA in Age and Time observation
...
```{r}
ggplot()+
 geom_polygon( data = all_states, aes(x = long, y=lat, group = group, fill=cont_scale), color="white", size = 0.2)+
  scale_fill_gradientn(colours = cm.colors(4),trans='log',name='count',
            breaks=c(0.00020,0.0004,0.001,0.005,0.01,0.05,0.1,0.3,0.6, 0.9), na.value = "grey50") 
...
plot<- ggplot(all_states02, aes(long, lat, group = group, fill=meanTime, colour= meanTime)) + geom_polygon(color="grey")+
   scale_fill_gradientn(colours = cm.colors(4),name='mean Timet',trans ="log", na.value = "grey50") +
with(centroids, 
       annotate(geom="text", x = long, y=lat, label = Hometown, 
                size = 4,color="Black",family="Times")
      )+
...
ggplot(by_year_sex, aes(x = DATE, y = mediantime, color = sex)) +
  ...
  ggtitle(label="Median race times over the years")
```
