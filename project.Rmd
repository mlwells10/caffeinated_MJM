...
test_complete$Time <- test_complete$Time %>% str_extract(pattern = "^\\d{1}:\\d{2}") %>%  #\d match any char with cat. number decimal digit
  hm()
test_complete <- rbind(test_complete,starts_5)
test_complete
```
...
```{r}
test_data = Wdata[(!is.na(Wdata$Age))&(!is.na(Wdata$Time)),] #removing NA in Age and Time observation
test_data <- test_data %>% filter(Time != "")
test_data <- test_data %>% filter(Time != "NR")

#test_data$Time = hms(test_data$Time) #convert time to hours, minutes, second
#test_data$Time = as.numeric(seconds(test_data$Time))/60.0
...
```{r}
test_1 <- test_data %>% #starts with 5
   filter(str_detect(Time, "^0")) %>% select(Time,rowname) 
test_2 <- test_data %>% #starts with 5
   filter(str_detect(Time, "^1")) %>% select(Time,rowname) 
test_3 <- test_data %>% #starts with 5
   filter(str_detect(Time, "^2")) %>% select(Time,rowname) 
test_complete <- rbind(test_1,test_2,test_3)
test_complete$Time <- test_complete$Time %>% str_extract(pattern = "^\\d{1}:\\d{2}") %>%  #\d match any char with cat. number decimal digit
  hm()
test_complete <- rbind(test_complete,testing)
test_complete
```
...
```{r}
test_data_joined <- left_join(test_data,test_complete, by="rowname")
