...
```{r}
model_3 <- lm(Time.y~0+PRCP,data = test_data)
summary(model_3)
```

PRCP is significant predictor but the Adj R-sq is low, as would be predicted. 
...
```{r}
model_5 <- lm(Time.y~0+TMIN,data = test_data)
summary(model_5)
```
...
```{r}
Wdat %>% arrange(Time.y) %>% filter(Race == 2015)
```
