...
ggplot(by_year_sex, aes(x = Race, y = mediantime, color = sex)) +
  ...
  ggtitle(label="Median race times over the years", subtitle= "Very few run times for women in 1978")
...
```{r}
boxcoxResult = boxcox(Time.y~poly(Age,3)*sex, data = data_w_new, lambda = seq(-2,5,1.5))
```

```{r}
lambda = boxcoxResult$x[which.max(boxcoxResult$y)]
data_w_new$yboxcox = (data_w_new$Time.y^lambda-1)/lambda
model.boxcox = lm(yboxcox~poly(Age,3)*sex, data = data_w_new)
...
```{r}
ols_model <- lm(yboxcox ~poly(Age,3)*sex+PRCP+TMIN+.data_M2024+.data_M2529+.data_M3034+.data_M3539+.data_M4044+.data_M4549+.data_M5054+.data_M5559+.data_M6064+.data_M6569+.data_M7074+.data_M7579+.data_W0119+.data_W2024+.data_W2529+.data_W3034+.data_W3539+.data_W4044+.data_W4549+.data_W5054+.data_W5559+.data_W6064, data = data_w_new)
```
...
```{r}
ols_step_forward_aic(ols_model,progress = T)
```
