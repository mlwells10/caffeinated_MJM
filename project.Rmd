...
comb_all_states <- all_states %>%
  rbind(df_cleantown, by=c("abb" ="Hometown"))
...
```{r}
library(MASS)
boxcoxResult = boxcox(Time.y ~ Age+sex, data =all_data, lambda = seq(-2,5,1.5))
```

```{r}
lambda = boxcoxResult$x[which.max(boxcoxResult$y)]
all_data$yboxcox = (all_data$Time.y^lambda-1)/lambda
model.boxcox = lm(yboxcox ~ 0+Age+sex,data = all_data)
