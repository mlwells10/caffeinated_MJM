...
agecut = cut(Wdat$Age, breaks = c(seq(15,75,10),90))
table(agecut)

plot(Wdat$Time.y ~ agecut, xlab = "Age (year)", ylab = "Run Time (minutes)",main = "Run Time for Subgroups of Runner With Same Age")

ggplot(Wdat )+
  geom_boxplot(aes(x = agecut, y =Time.y))+ 
  xlab("Age (year)")+
  ylab("Run Time (minutes)")+
  labs(title = "Run Time for Subgroups of Runner", subtitle = "With Roughly the Same Age")
```
...
df_cluster_2 <- df_cleantown %>% select(Hometown, Age, Time.y)
