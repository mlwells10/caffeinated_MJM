...
#Only 2 values for 1973 that were not NA Age
...
```{r}
Wdat = Wdat[(!is.na(Wdata$Time)),] #removing NA in Age and Time observation
...
```{r}
Mdat = Mdat[(!is.na(Mdata$Time)),] #removing NA in Age and Time observation
...
test_1_M %>% filter(Time == "0.047581019")
```
...
```{r}
all_data %>% filter(Race == 1978) %>% select(Time.y,sex) %>% arrange(Time.y)
```
...
```{r}
boxcoxResult = boxcox(Time.y ~ Division, data =data_w, lambda = seq(-2,5,1.5))
```

```{r}
lambda = boxcoxResult$x[which.max(boxcoxResult$y)]
data_w$yboxcox = (data_w$Time.y^lambda-1)/lambda
model.boxcox = lm(yboxcox ~ Division,data = data_w)
