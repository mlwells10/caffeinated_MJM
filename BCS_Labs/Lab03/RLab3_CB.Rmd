...
dat <- dat[is.na(dat$Age) != T & is.na(dat$Time) != T,]
dat$Time = hms(dat$Time)
dat$Time = as.numeric(seconds(dat$Time)) / 60
...
ggplot(data=dat,aes(x=Time)) + geom_histogram(binwidth=5) + facet_wrap(Year~.)+theme(axis.text.x = element_text())+xlab("Time (minutes)")+ylab("Counts")+ggtitle("Race times every year")
ggplot(data=dat,aes(x=Time)) + geom_density() + facet_wrap(Year~.)+theme(axis.text.x = element_text())+xlab("Time (minutes)")+ggtitle("Race times every year")
```
...
```{r}
dat %>% ggplot(aes(x = Age, y = Time, color = Year))+
  geom_point()+
