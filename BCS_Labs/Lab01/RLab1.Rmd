...
```{r}
#Variables that doesn't provide no informariona 
...
<<<<<<< HEAD
```{r}
signal_summary <- function(posX, posY, posZ, orientation, mac){
  obs = relat_access02[relat_access02$posX == posX & relat_access02$posY == posY & relat_access02$posZ == posZ & rec_orient == orientation & relat_access02$mac == mac,]
  return(c(mean(obs$signal),sd(obs$signal)))  
}
signal_summary(2,0,0,0,"00:14:bf:b1:97:8a")
```
...
```{r}
signal_summary <- function(dat){
  subset_data <- relat_access02[relat_access02$posX == dat[1] & relat_access02$posY ==dat[2] & relat_access02$posZ == dat[3] & relat_access02$orientation == dat[4] & relat_access02$mac == dat[5],]
  
  return(c(mean(subset_data$signal), sd(subset_data$signal)))
}

signal_summary(c(2,0,0,0,'00:14:bf:b1:97:8a'))
```
...
#apply signal_summary to all possible combinations
apply(grid,1,signal_summary)
>>>>>>> c6607c77fc5c0035f70c84a42c2acedc396b8369
```
